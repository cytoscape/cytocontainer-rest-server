...
def update_network(
    ...
    err_stream.write('@@MESSAGE Adding createdBy attribute\n')

    net.add_network_attribute(key="createdBy",
                              value=createdby, datatype="string")

    return [net.to_cx2(), net.to_cx2()]


def networks_to_json(data: list, err_stream=sys.stderr) -> Dict[str, Any]:
    """
    Wrap the CX2 payload in a CyWeb-compatible addNetworks action.
    """
...
def run_addnetworks(
    ...
    updated = update_network(net,createdby=createdby)
    result = networks_to_json(updated)
