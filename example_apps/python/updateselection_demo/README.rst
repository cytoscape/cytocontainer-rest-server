========================================================================
UpdateSelection Cytoscape Container REST Service Demo Service App
=========================================================================

This minimal Python service demonstrates how to accept a CX2 network and
return an updated selection of nodes payload through the CyWeb
``updateSelection`` action. It is intended as a starting template for Cytoscape
...
* Adds a network attribute ``updatedBy = updateselection_demo``
* Emits the modified CX2 wrapped in a single ``updateSelection`` action
...
.. code-block:: bash
   ...
   cd example_apps/python/updateselection_demo
   python -m venv .venv
...
.. code-block:: bash
   ...
   cytoscape-updateselection-demo foo.cx2
...
From the *updateselection_demo* directory (same level as ``pyproject.toml``) run:
...
Above will create `cytoscape-updateselection-demo:latest` image. For additional configuration options
...
.. code-block:: bash
   ...
   docker run --rm cytoscape-updateselection-demo:latest
...
.. code-block:: bash
   ...
   docker run --rm \
     -v "$(pwd)":/data \
     cytoscape-updateselection-demo:latest \
     /data/foo.cx2

The selection JSON will be output to stdout


Integration with Cytocontainer
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Copy ``python_updateselectiondemo.json`` directory to the directory specified by 
...
The app will be available at the endpoint ``/v1/algorithms/python_updateselectiondemo`` 
